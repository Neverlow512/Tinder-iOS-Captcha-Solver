...
# Reconfigure logging to include file handlers now that LOGS_DIR is set
...
logging.basicConfig(
    ...
    format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[
        logging.FileHandler(os.path.join(LOGS_DIR, "general.log")),
        logging.StreamHandler(sys.stdout)
    ]
)
...
captcha_handler = logging.FileHandler(os.path.join(LOGS_DIR, '2captcha_interactions.log'))
        ...
        while True:
            if detect_captcha(driver):
                ...
                if success:
                    ...
                    # Delete the session directory after captcha is solved
